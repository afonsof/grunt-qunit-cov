...
module.exports = function(grunt)
{
    ...
    grunt.registerMultiTask('qunit-cov', 'Run QUnit unit tests in a headless PhantomJS instance.', function()
    {
        ...
        {
            var dir = depDirs[i];
            grunt.log.ok('Copy dir ' + dir + ' to ' + outDir + '/in/' + dir);            
            copyDirSyncRecursive(dir, outDir + '/in/' + dir);
        }
    
        grunt.log.writeln('Instrumenting ' + srcDir + ' to ' + outDir + '/' + srcDir);
        ...
        grunt.helper('jscoverage',
        {
            ...
            {
                grunt.log.writeln('Instrumenting ended');
    ...
    function PrintCoverage(coverageInfo, minimum, srcDir, outDir)
    {
        ...
        for(var key in coverageInfo)
        {
            ...
            var lineCoverage = coverageInfo[key];
            var originalFile = srcDir + "/" + key;
            var fileLines = grunt.file.read(originalFile).split(/\r?\n/);
